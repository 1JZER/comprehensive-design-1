...
package com.zuel.filter;


import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.zuel.config.Config;
import com.zuel.dto.GatewayErrorResult;
...
public class TokenValidateGatewayFilterFactory extends AbstractGatewayFilterFactory<Config> {
        ...
        return ((exchange, chain) -> {
            ...
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {
                ...
                if (StringUtils.hasText(username) && StringUtils.hasText(token) &&
                        (userInfo = stringRedisTemplate.opsForHash().get("comprehensive-design:login:" + username, token)) != null) {
                ...
                // token不匹配, 返回Token validation error
                ServerHttpResponse response = exchange.getResponse();
                response.setStatusCode(HttpStatus.UNAUTHORIZED);
                return response.writeWith(Mono.fromSupplier(() -> {
                    DataBufferFactory bufferFactory = response.bufferFactory();     // 数据缓冲工厂
    ...
    private boolean isPathInWhiteList(String requestPath, String requestMethod, List<String> whitePathList) {
        return (
                !CollectionUtils.isEmpty(whitePathList)         // 白名单不为空
                        && whitePathList.stream().anyMatch(requestPath::startsWith))
                || (Objects.equals(requestPath, "/api/comprehensive-design/admin/v1/user")
